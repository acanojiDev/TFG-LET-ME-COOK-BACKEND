...
export class ViewService {
     ...
     */
    static async markPostAsViewed(user_id: string, post_id: string) {
        return prisma.user_viewed_posts.upsert({
            where: {
                user_id_post_id: {
                    user_id: user_id,
                    post_id: post_id
     ...
     */
    static async getViewedPosts(user_id: string) {
        return prisma.user_viewed_posts.findMany({
            where: {
                user_id: user_id
            },
            include: {
     ...
     */
    static async hasUserViewedPost(user_id: string, post_id: string) {
        const view = await prisma.user_viewed_posts.findUnique({
            where: {
                user_id_post_id: {
                    user_id: user_id,
                    post_id: post_id
     ...
     * @param post_id ID del post
     */
    static async unmarkPostAsViewed(user_id: string, post_id: string) {
        return prisma.user_viewed_posts.delete({
            where: {
                user_id_post_id: {
                    user_id: user_id,
                    post_id: post_id
