...
export const resolvers = {
	Mutation: {
		register: async (_: any, args: any, ctx: Context) => {
			...
			// 2. Escribir user_type en app_metadata de Supabase
			// Esto evita la query a Prisma en cada request para obtener el user_type
			// app_metadata es gestionado por el servidor (no modificable por el cliente)
			if (supabaseAdmin) {
			...
			// 3. Crear registro en nuestra BD
		...
		login: async (_: any, args: any, ctx: Context) => {
			...
			// Asegurar que app_metadata tiene user_type (usuarios creados antes de esta migración)
			const metaUserType = authData.user.app_metadata?.user_type;
			if (!metaUserType && supabaseAdmin) {
				await supabaseAdmin.auth.admin.updateUserById(userId, {
					app_metadata: { user_type: dbUser.user_type }
				});
			}
